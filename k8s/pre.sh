...
echo "$current_time ==》删除 service" >> $path/output.log 2>&1
...
echo "$current_time ==》暂停并删除镜像为$imageName 的容器" >> $path/output.log 2>&1
containers=$(docker ps -aq --filter ancestor=$imageName)
if [ -n "$containers" ]; then
    echo "$current_time ==》停止容器：$containers" >> $path/output.log 2>&1
    docker stop $containers >> $path/output.log 2>&1
    echo "$current_time ==》删除容器：$containers" >> $path/output.log 2>&1
    docker rm $containers >> $path/output.log 2>&1
    # 删除镜像
    echo "$current_time ==》删除镜像：$imageName" >> $path/output.log 2>&1
